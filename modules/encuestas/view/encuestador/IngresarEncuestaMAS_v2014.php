...
if ($estado2==8)
{
	$strsql="select * from qs_encuestascli_masgarantia_v2014 
	...
	if ($result)
	{
		...
		$url="EditarEncuestaMAS_v2014.php?id_encuesta=".$row['id_encuesta']."&id_formato=".$id_formato."&estado=".$estado;
		Header("Location: $url");	
	}
	else
	{
		$url="EditarEncuestaMAS_v2014.php?id_encuesta=".$id_encuesta."&id_formato=".$id_formato."&estado=".$estado;
		Header("Location: $url");	
	}
}
...
$strsql="select * from qs_encuesta_masgarantia_v2014 where id_encuesta = '".$id_encuesta."'";
$result=consulta_bd($strsql);

if ($result)
{
	$StrSql="UPDATE qs_encuesta_masgarantia_v2014 set 
	preg1_a = '".$preg1_a."', 
	preg1_b = '".$preg1_b."', 
	preg1_c = '".$preg1_c."', 
	preg2 = '".$preg2."', 
...
{
	$StrSql="INSERT INTO qs_encuesta_masgarantia_v2014 (id_encuesta,
...
if(query_bd($StrSql))
{
		$StrSql="UPDATE qs_encuestascli_masgarantia_v2014 set estado = ".$estado2." 
		...
		if ($_POST['enviar']=='POSTERGAR ENCUESTA') {
			if ($num_post >= 6) {
				$StrSql="UPDATE qs_encuestascli_masgarantia_v2014 set estado = 9, num_post = ".$num_post."  + 1  
				where id_encuesta = ".$id_encuesta;
				query_bd($StrSql);
			}
			else
			{
				$StrSql="UPDATE qs_encuestascli_masgarantia_v2014 set fec_postergada = '".$fec_postergada_format."'  
				, num_post = ".$num_post."  + 1 
				where id_encuesta = ".$id_encuesta;
				query_bd($StrSql);
			}
		...
		VALUES (".$_SESSION['s_id_acceso'].",".$_SESSION['s_id_acceso'].", NOW() , 'Encuesta MAS GARANTIA v2014 Ingresada por encuestador ".$_SESSION['s_id_acceso']."')";
		query_bd($StrSql);
