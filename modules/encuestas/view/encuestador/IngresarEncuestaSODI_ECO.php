...
if ($estado2==8)
{
	$strsql="select * from qs_encuestascli_sodimac_eco 
	...
	if ($result)
	{
		...
		$url="EditarEncuestaSODI_ECO.php?id_encuesta=".$row['id_encuesta']."&id_formato=".$id_formato."&estado=".$estado;
		Header("Location: $url");	
	}
	else
	{
		$url="EditarEncuestaSODI_ECO.php?id_encuesta=".$id_encuesta."&id_formato=".$id_formato."&estado=".$estado;
		Header("Location: $url");	
	}
}
...
$StrSql="UPDATE qs_encuestascli_sodimac_eco set 
Id1  = '".$Id1."', 
...
$strsql="select * from qs_encuesta_sodimac_eco where id_encuesta = '".$id_encuesta."'";
$result=consulta_bd($strsql);
...
{
	$StrSql="INSERT INTO qs_encuesta_sodimac_eco (id_encuesta,
	preg1,
	preg2,
	preg3,
	preg4,
...
if(query_bd($StrSql))
{
		$StrSql="UPDATE qs_encuestascli_sodimac_eco set estado = ".$estado2." 
		...
		if ($_POST['enviar']=='POSTERGAR ENCUESTA') {
			if ($num_post >= 6) {
				$StrSql="UPDATE qs_encuestascli_sodimac_eco set estado = 9, num_post = ".$num_post."  + 1  
				where id_encuesta = ".$id_encuesta;
				query_bd($StrSql);
			}
			else
			{
				$StrSql="UPDATE qs_encuestascli_sodimac_eco set fec_postergada = '".$fec_postergada_format."'  
				, num_post = ".$num_post."  + 1 
				where id_encuesta = ".$id_encuesta;
				query_bd($StrSql);
			}
