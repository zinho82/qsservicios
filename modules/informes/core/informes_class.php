...
group by month(cda.fresp) ";
        ...
        while ($mall = mysql_fetch_array($res)) {
            ...
            $neu += $mall['qneu'];
            $pos += $mall['qpos'];
            $nps = (($mall['qpos'] / $mall['npst']));
            $qtot = $mall['qneg']+$mall['qneu']+$mall['qpos'];
            $qtotalAc+=$qtot;
//            $npsto = (($npspt / $npst) - ($npsn / $npst));
            echo "['" . $conn->MesRecortado($mall['Mes']) . '-' . date('y') . "',-" . (($mall['qneg'] / $qtot) * 100) . "," . (($mall['qneu'] / $qtot) * 100) . "," . (($mall['qpos'] / $qtot) * 100) . "," . ($nps) . "],";
        }
        echo "['Acum',-".(($neg/$qtotalAc)*100).",".(($neu/$qtotalAc)*100).",".(($pos/$qtotalAc)*100).",0]";
    }  
